...
(defn scale
  ...
  (case (:kind shape)
    ...
    (throw (#?(:clj Exception. :cljs js/Error.) (str "Can't scale non-shape: " (pr-str shape))))))
...
(defn position
  ...
  (case (:kind shape)
    ...
    (throw (#?(:clj Exception. :cljs js/Error.) (str "Can't position non-shape: " (pr-str shape))))))
...
(defn ^BBox bbox ^BBox [^Shape shape]
  ...
  (case (.-kind shape)
    ...
    (throw (#?(:clj Exception. :cljs js/Error) (str "Can't take bbox of non-shape: " (pr-str shape))))))
...
#?(:cljs (defn value-to-cell! [the-cell & cell-path]
           (fn [^js event]
